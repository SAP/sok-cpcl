...
def puma_softmax(self, dim, **kwargs):
    r"""Compute the softmax of a tensor's elements along a given dimension
    using the approximation of the softmax function proposed by PUMA (Dong et al.2023).


    Approximates the negative exponential function
    computing the exponentiation via Taylor series with simple clipping.

    if x < Texp:
        neg_exp(x) = 0
    elif Texp <= x <= 0:
        neg_exp(x) = (1 + x/2**t)**(2*t) 

    x/2**t can be computed by shifting the exponent of x by -t.
    the exponentiation can be computed as t times squaring of (1 + x) and truncation.
    """
    ...
    eps = 1e-6
    maximum_value = self.max(dim, keepdim=True)[0]
    logits = self - maximum_value - eps

    Texp = cfg.functions.neg_exp_threshold
    # exp on large negative is clipped to zero
    b = logits > - Texp
    nexp = logits.exp() * b
    divisor = nexp.sum(dim, keepdim=True)

    return nexp / divisor
    
    #t = cfg.functions.neg_exp_threshold
    #lt = self < Texp
    #inv_2t = pow(2, -t)
    #z = crypten.CrypTensor(torch.zeros(t))
    #z[0] = 1 + self.mul(inv_2t)
...
def softmax_maxcap(self, dim, max_value=100, **kwargs):
    f""" Compute the softmax of a tensor's elements along a given dimension
    w/o the computation of the maximum value of the input tensor.
    It caps the input tensor to a maximum value to avoid numerical instability.
    """
    ...
    cap_logits = tanh(self.div(max_value)).mul(max_value)
    logits = cap_logits - max_value
    numerator = logits.exp()
    with cfg.temp_override({"functions.reciprocal_all_pos": True}):
        inv_denominator = numerator.sum(dim, keepdim=True).reciprocal()
    return numerator * inv_denominator