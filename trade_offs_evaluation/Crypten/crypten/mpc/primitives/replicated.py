...
def _replicate_shares(share_list):
    world_size = comm.get().get_world_size()
    if world_size < 3:
        raise ValueError("Cannot utilize Replicated Sharing securely with < 3 parties.")
    rank = comm.get().get_rank()
    ...
    rep_shares = []
    for share in share_list:
        rep_shares.append(torch.zeros_like(share.data))

        send_req = comm.get().isend(share.data, dst=next_rank)
        recv_req = comm.get().irecv(rep_shares[-1].data, src=prev_rank)
    ...
    # Order [(x1, x2), (y1, y2), ...]
    shares = [(share_list[i], rep_shares[i].to(share_list[i].device)) for i in range(len(share_list))]

    return shares

def _replicate_shares(
    x_share,
    rank,
    world_size,
    prev_rank,
