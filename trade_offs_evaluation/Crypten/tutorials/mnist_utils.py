...
def split_observations(
    ...
    # Labes have to be saved before, otherwise .long() change the value type
    torch.save(party1_train_labels.float(), os.path.join(dir, party1 + "_train_labels.pth"))
    torch.save(party1_test_labels.float(), os.path.join(dir, party1 + "_test_labels.pth"))
    torch.save(party2_train_labels.float(), os.path.join(dir, party2 + "_train_labels.pth"))
    torch.save(party2_test_labels.float(), os.path.join(dir, party2 + "_test_labels.pth")) 

    # From Here modified to load directly the one hot encoded labels
    party1_train_label_eye = torch.eye(2)
    party2_train_label_eye = torch.eye(2)
    party1_test_label_eye = torch.eye(2)
    party2_test_label_eye = torch.eye(2)
    party1_train_labels_onehot = party1_train_label_eye[party1_train_labels.long()]
    party2_train_labels_onehot = party2_train_label_eye[party2_train_labels.long()]
    party1_test_labels_onehot = party1_test_label_eye[party1_test_labels.long()]
    party2_test_labels_onehot = party2_test_label_eye[party2_test_labels.long()]
   
    
    # Here one hot encoded save
    torch.save(party1_train_labels_onehot, os.path.join(dir, party1 + "_train_labels_onehot.pth"))
    torch.save(party1_test_labels_onehot, os.path.join(dir, party1 + "_test_labels_onehot.pth"))
    torch.save(party2_train_labels_onehot, os.path.join(dir, party2 + "_train_labels_onehot.pth"))
    torch.save(party2_test_labels_onehot, os.path.join(dir, party2 + "_test_labels_onehot.pth"))
