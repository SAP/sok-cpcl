...
class DPAdamW(Optimizer):
    ...
    def __init__(
        ...
        super(DPAdamW, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(DPAdamW, self).__setstate__(state)
        ...
        with crypten.no_grad():
            ...
            for group in self.param_groups:
                ...
                for i, p in enumerate(group["params"]):
                    ...
                    if len(state) == 0:
                        state["step"] = 0
                        state["exp_avg"] = crypten.cryptensor(torch.zeros(p.size()))
                        state["exp_avg_sq"] = crypten.cryptensor(torch.zeros(p.size()))
                    ...
                    exp_avg = exp_avg.mul(beta1).add(d_p.mul(1 - beta1))
                    exp_avg_sq = exp_avg_sq.mul(beta2).add(d_p.square().mul(1 - beta2))

                    denom = (exp_avg_sq.add(eps)).inv_sqrt()

                    num = exp_avg/(1-beta1**t)
                    denom = denom/(1-beta2**t)

                    d_p = (num.mul(denom) + dp_noise.to(self.device)) / batch_size
