...
class BaseAutogradContext(object):
    ...
    def save_for_ghost_clipping(self, grad_output):
        pass
...
class AutogradFunction(object):
    ...
    HERE: Added the forward and backward functions for the hooks
    """
    def __init__(self):
        self._forward_hooks = []
...
# HERE added softmax_maxcap and puma_softmax
@register_function("softmax_maxcap")
class AutogradSoftmaxMax(AutogradFunction):
    @staticmethod
    def forward(ctx, input, dim, max_value):
        probs = input.softmax_maxcap(dim, max_value)
        ctx.save_multiple_for_backward([probs, dim])
        return probs
...
@register_function("puma_softmax")
class AutogradPumaSoftmax(AutogradFunction):
    @staticmethod
    def forward(ctx, input, dim):
        probs = input.puma_softmax(dim)
        ctx.save_multiple_for_backward([probs, dim])
        return probs
...
"""
Newly added autograd for layer normalization
"""
...
class AutogradLayerNorm(AutogradFunction):
    ...
    def forward(
        ...
        Args:
            ctx (autograd_cyptensor.AutogradContext): context which
                stores parameters such as weight and bias for backward step.
            x (cryptensor): input tensor of shape `(B, +, normalize_shape)`.
            normalized_shape (tuple): shape of the layer norm dimensions.
            weight (cryptensor): weight of shape normalized_shape.
            bias (cryptensor): bias of shape normalized_shape.
            eps (float): specifies epsilon used for numerical precision in inv_var

        Returns: (weight * normalized input + bias) of shape `(B, +, normalize_shape)`.
        """
        end_index = x.dim()
        start_index = end_index - len(normalized_shape)
        stats_dimensions = list(range(start_index, end_index))
        

        mean = x.mean(dim = stats_dimensions, keepdim=True) 
        ...
        variance = (x - mean).pow(2).mean(dim = stats_dimensions, keepdim=True)
        

        # This variance returned only one value 
        #variance = x.var(dim = stats_dimensions, unbiased=False, keepdim=True)
        # HERE using inverse of sqrt of variance
        inv_var = (variance + eps).inv_sqrt()
        # HERE: saving pre computations for backward
        x_norm = (x - mean) * inv_var
        ctx.save_multiple_for_backward([x_norm, inv_var, weight, normalized_shape])
        return x_norm * weight + bias
    ...
    def backward(ctx, grad_output):
        ...
        Args:
            ctx (autograd_cyptensor.AutogradContext): context containing:
                x_norm (cryptensor): normalized input x of shape (B, +, normalized_shape)
                inv_var (cryptensor): inverse of the square root of the variance 
                weight (cryptensor): weight of shape normalized_shape.
                normalized_shape (tuple): shape of the layer norm dimensions.
            grad_output (cryptensor): layernorm output of shape (B, +, normalized_shape)

        Returns:
            grad_x (cryptensor): gradient with respect to x with shape (B, +, normalized_shape).
            grad_weight (cryptensor): gradient with respect to the weight of
                with normalized_shape.
            grad_bias (cryptensor): gradient with respect to bias of normalized_shape.
        """

        # retrieve context:
        x_norm, inv_var, weight, normalized_shape = ctx.saved_tensors

        end_index = grad_output.dim()
        start_index = end_index - len(normalized_shape)
        stats_dimensions_param = list(range(0, start_index))
        stats_dimensions = list(range(start_index, end_index))

        broadcast_shape = [1] * grad_output.dim()
        for i in stats_dimensions_param:
            broadcast_shape[i] = grad_output.shape[i]
        ...
        # compute gradient w.r.t. weight:
        grad_weight = grad_output.mul(x_norm)
        grad_weight = grad_weight.sum(dim=stats_dimensions_param)

        # compute gradient w.r.t. bias:
        grad_bias = grad_output.sum(dim=stats_dimensions_param)
        #grad_bias = None
        ...
        grad_mean = grad_output.sum(dim=stats_dimensions)
        grad_mean = grad_mean.reshape(broadcast_shape)
        grad_mean = grad_mean.mul(inv_var.div(-num_element))

        grad_std = x_norm.mul(grad_output).sum(dim=stats_dimensions)
        grad_std = grad_std.reshape(broadcast_shape)
        grad_std = x_norm.mul(grad_std).mul(inv_var.div(-num_element))
...
#HERE: registered linear for the hooks
@register_function("linear")
class AutogradLinear(AutogradFunction):
    @staticmethod
    def forward(ctx, input, weight, bias=None):
        #super(AutogradLinear, ctx).forward(input)
        ctx.reset()
        ctx.save_multiple_for_backward([input, weight])
        if bias is not None:
            return input.matmul(weight.t()).add(bias)
        return input.matmul(weight.t()).add(bias)
    ...
    def backward(ctx, grad_output):
        ...
        ctx.save_for_ghost_clipping(grad_output)
        #super(AutogradLinear, ctx).backward(grad_input, grad_output)
        return grad_input, grad_weight, grad_bias  