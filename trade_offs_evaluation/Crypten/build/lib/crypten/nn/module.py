...
class Module:
    ...
    # HERE: add hooks dictionaries to support hooks
    ...
    def __init__(self):
        ...
        # HERE: removed to avoid errors in LoRA layers __init__
        #self.train()
        # HERE: add training attribute to avoid errors during training with LoRA layers
        self.training = False
    ...
    # HERE: add hooks methods to support hooks
    ...
    def _call_impl(self, *args, **kwargs):
        ...
        if not (self._backward_hooks or self._forward_hooks):
...
class Embedding(Module):
    def __init__(self, x, y):
        super().__init__()
        pytorch_module = torch.nn.Embedding(x, y)

        self.register_parameter("weight", pytorch_module.weight)

    def forward(self, input, inputs_embeds=None):
        
        # if input is encrypted, then the embedding can be 
        # retrieved only via matrix multiplication
        if isinstance(input, torch.Tensor):
            return self.weight[input]
        # The input has to be ONE HOT encoded
        return input.matmul(self.weight)
    
    @staticmethod
    def from_onnx(attributes=None):
        return Embedding(attributes["input_dim"], attributes["output_dim"])
...
class LayerNorm(Module):
    def __init__(self, normalized_shape, eps=1e-12, elementwise_affine=True):
        ...
        for param in ["weight", "bias"]:
            self.register_parameter(param, getattr(pytorch_module, param))
        #self.register_parameter("weight", (torch.zeros(normalized_shape)))
        #self.register_parameter("bias", (torch.zeros(normalized_shape)))

        # Parameters in Crypten are modules, whereas in PyTorch they are tensors
        # So to get the data of the parameters you need to do self.weight.data
        #self.weight = cnn.Parameter(torch.empty(normalized_shape))
        #self.bias = cnn.Parameter(torch.empty(normalized_shape))
        
        #self.running_mean = cnn.Parameter(torch.zeros(normalized_shape))
        #self.running_var = cnn.Parameter(torch.ones(normalized_shape))
        
        # Eps only serves for numerical stability to the denominator
        self.eps = eps
    ...
    def from_onnx(attributes=None):
        if attributes is None:
            attributes = {}
        # TODO: Conversion from ONNX, 
        #       ONNX has axis which is the first normalization axis
        #       PyTorch has normalized_shape  
        #       Save weight and biases  
        return LayerNorm(
            normalized_shape=attributes.get("axis", 1),
            eps=attributes.get("epsilon", 1e-05),
        ) 



#HERE added LoRALinear
class LoRALinear(Module):
    ...
    def __init__(
        ...
        if state_dict is not None:
            # Copy the values for the weight and bias
            self.weight = state_dict["weight"]
            self.bias = state_dict["bias"]

        if self.r > 0:
            torch_weight = torch.nn.Linear(in_features, out_features).weight
            self.register_parameter('lora_A', torch.nn.Parameter(torch_weight.new_zeros((r, in_features))))
            self.register_parameter('lora_B', torch.nn.Parameter(torch_weight.new_zeros((out_features, r))))
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, 'lora_A'):
            ...
            if "relu" not in self.non_linearity.lower():
                torch.nn.init.normal_(self.lora_A, 0, 1)
            else:
                torch.nn.init.kaiming_normal_(self.lora_A, nonlinearity=self.non_linearity)
    ...
    def encrypt(self, mode=True, src=0):
        ...
        if self.r > 0:
            # Decrypt the linear layer (weight and bias)
            if mode:
                if isinstance(self.weight, crypten.CrypTensor):
                    self.weight = self.weight.get_plain_text()
                    if self.update_bias == 'none':
                        self.bias = self.bias.get_plain_text()
                    if self.freeze_A:
                        self.lora_A = self.lora_A.get_plain_text()
    ...
    def reset_linear_parameters(self):
        ...
        if self.bias is not None:
            fan_in = self.weight.shape[1] if self.weight.dim() > 1 else 1
            bound = 1 / pow(fan_in, 0.5)
            torch.nn.init.uniform_(self.bias, -bound, bound)
    
    #def train(self, mode: bool = True):
    #    def T(w):
    #        return w.transpose(0, 1) if self.fan_in_fan_out else w
    #    #cnn.Linear.train(self, mode)
    #    if mode:
    #        if self.merge_weights and self.merged:
    #            # Make sure that the weights are not merged
    #            if self.r > 0:
    #                self.weight.sub(T((self.lora_B).matmul(self.lora_A)) * self.scaling)
    #            self.merged = False
    #    else:
    #        if self.merge_weights and not self.merged:
    #            # Merge the weights and mark it
    #            if self.r > 0:
    #                self.weight.add(T((self.lora_B).matmul(self.lora_A)) * self.scaling)
    #            self.merged = True       

    def forward(self, input):
        def T(w):
            return w.transpose(0, 1) if not self.fan_in_fan_out else w
        
        if self.r > 0 and not self.merged:
            result = input.matmul(T(self.weight)) + self.bias
            result = result.add(((self.lora_dropout(input)).matmul(self.lora_A.transpose(0, 1))).matmul(self.lora_B.transpose(0, 1)) * self.scaling)
            #result = F.linear(x, T(self.weight), bias=self.bias)            
            #result += (self.lora_dropout(x) @ self.lora_A.transpose(0, 1) @ self.lora_B.transpose(0, 1)) * self.scaling
            return result
        else:
            return input.matmul(T(self.weight)) + self.bias
...
#HERE added PumaSoftmax
class PumaSoftmax(Module):
    r"""
    Approximation from PUMA paper for the softmax function
    ...
    def forward(self, input):
        return input.puma_softmax(self.dim)
    

# HERE added softmax_maxcap
class SoftmaxMaxCap(Module):
    r"""Applies the Softmax function to an n-dimensional input Tensor
    rescaling them so that the elements of the n-dimensional output Tensor
    lie in the range [0,1] and sum to 1.

    This version performs logits capping before applying the softmax function.
    ...
    def forward(self, input):
        return input.softmax_maxcap(self.dim, self.max_value)
